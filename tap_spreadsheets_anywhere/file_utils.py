...
def sample_file(table_spec, target_filename, sample_rate, max_records):
    LOGGER.info(
        "Sampling {} ({} records, every {}th record).".format(
            target_filename, max_records, sample_rate
        )
    )
...
def sample_files(
    table_spec, target_files, sample_rate=10, max_records=1000, max_files=5
):
    to_return = []
    ...
    for target_file in target_files:
        to_return += sample_file(
            table_spec, target_file["key"], sample_rate, max_records
        )
        files_so_far += 1
...
def parse_path(path):
    ...
    return (
        ("local", path_parts[0])
        if len(path_parts) <= 1
        else (path_parts[0], path_parts[1])
    )
...
def get_matching_objects(table_spec, modified_since=None):
    ...
    if protocol == "s3":
        target_objects = list_files_in_s3_bucket(
            bucket, table_spec.get("search_prefix")
        )
    elif protocol == "file":
        target_objects = list_files_in_local_bucket(
            bucket, table_spec.get("search_prefix")
        )
    elif protocol in ["sftp"]:
        target_objects = list_files_in_SSH_bucket(
            table_spec["path"], table_spec.get("search_prefix")
        )
    elif protocol in ["ftp"]:
        target_objects = list_files_in_ftp_server(
            table_spec["path"], table_spec.get("search_prefix")
        )
    elif protocol in ["gs"]:
        target_objects = list_files_in_gs_bucket(
            bucket, table_spec.get("search_prefix")
        )
    ...
    if modified_since:
        LOGGER.info(
            f'Checking {len(target_objects)} resolved objects for any that match regular expression "{pattern}" and were modified since {modified_since}'
        )
    else:
        LOGGER.info(
            f'Checking {len(target_objects)} resolved objects for any that match regular expression "{pattern}"'
        )
    ...
    for obj in target_objects:
        ...
        if matcher.search(key) and (
            ...
            LOGGER.debug(
                ...
                + " comparing to {} ".format(modified_since)
            )
    ...
    if not LOGGER.isEnabledFor(logging.DEBUG):
        LOGGER.info(
            f"Processing {len(to_return)} resolved objects that met our criteria. Enable debug verbosity logging for more details."
        )
...
def convert_URL_to_file_list(table_spec):
    ...
    if r:
        if "last-modified" in r.headers:
            ...
            LOGGER.warning(
                "URL did not return a last-modified header so using current date and time."
            )
...
def list_files_in_ftp_server(uri, search_prefix=None):
    ...
    ftp = ftp_transport._connect_ssh(
        parsed_uri["host"],
        parsed_uri["user"],
        parsed_uri["port"],
        parsed_uri["password"],
...
def list_files_in_local_bucket(bucket, search_prefix=None):
    ...
    for dirpath, dirnames, filenames in walk(path, onerror=raise_error):
        ...
        if len(local_filenames) > max_results:
            raise ValueError(
                f"Read more than {max_results} records from the path {path}. Use a more specific "
                f"search_prefix"
            )
    ...
    return [
        {
            "Key": filename,
            "LastModified": datetime.fromtimestamp(
                os.path.getmtime(os.path.join(path, filename)), timezone.utc
            ),
        }
        for filename in local_filenames
...
def list_files_in_gs_bucket(bucket, search_prefix=None):
    ...
    target_objects = [
        {"Key": blob.name, "LastModified": blob.updated} for blob in blobs
    ]
...
def list_files_in_s3_bucket(bucket, search_prefix=None):
    ...
    if result["KeyCount"] > 0:
        ...
        while next_continuation_token is not None:
            LOGGER.debug(
                'Continuing pagination with token "{}".'.format(next_continuation_token)
            )
...
def config_by_crawl(crawl_config):
    ...
    for source in crawl_config:
        ...
        modified_since = dateutil.parser.parse(
            source["start_date"] if "start_date" in source else "1970-01-01T00:00:00Z"
        )
            ...
            if not file["key"].endswith("/"):
                ...
                if table not in entries:
                    entries[table] = {
                        ...
                        "prefer_number_vs_integer": source.get(
                            "prefer_number_vs_integer", False
                        ),
                        "prefer_schema_as_string": source.get(
                    ...
                    if table_with_pattern not in entries:
                        entries[table_with_pattern] = {
                            ...
                            "invalid_format_action": "ignore",
                            "delimiter": "detect",
                            "max_records_per_run": source.get(
                                "max_records_per_run", -1
                            ),
                            "max_sampled_files": source.get("max_sampled_files", 5),
                            "max_sampling_read": source.get("max_sampling_read", 1000),
                            "universal_newlines": source.get(
                ...
                LOGGER.debug(
                    f"Skipping config for {file['key']} because it looks like a folder not a file"
                )
